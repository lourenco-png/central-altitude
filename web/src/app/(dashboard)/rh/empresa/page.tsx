...
import {
  Save, Plus, Trash2, Building2, FileText,
  AlertTriangle, ExternalLink, Upload, ChevronDown, ChevronRight, User,
} from 'lucide-react';
...
import type { Empresa, Socio, DocumentoSocio } from '@/types';
...
// ── Seção de documentos de um sócio ────────────────────────────────────────
...
function SocioCard({
  ...
  socio: Socio;
  onRemoveSocio: () => void;
  onAddDoc: (socioId: string, data: any) => void;
    ...
    <div className="border border-neutral-200 rounded-xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-neutral-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-neutral-900">{socio.nome}</p>
            <p className="text-xs text-neutral-400">
              {socio.cargo || '—'}{socio.cpf ? ` · CPF: ${socio.cpf}` : ''}
            </p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <button
            onClick={() => setOpen(o => !o)}
            className="flex items-center gap-1 text-xs text-primary-600 px-2 py-1 rounded hover:bg-primary-50 transition-colors"
          >
            <FileText size={13} />
            {docs.length > 0 ? `${docs.length} doc(s)` : 'Documentos'}
            {open ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
          </button>
          <button
            onClick={onRemoveSocio}
      ...
      {open && (
        <div className="p-4 border-t border-neutral-100 grid grid-cols-1 md:grid-cols-2 gap-4">

          {/* Formulário de adição */}
          <div className="bg-primary-50 border border-primary-100 rounded-lg p-4">
            <p className="text-xs font-semibold text-primary-800 mb-3 flex items-center gap-1.5">
            ...
            <form onSubmit={handleSubmitDoc} className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: RG, CPF, Procuração..."
                />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    type="date"
                    value={docForm.emissao}
                    onChange={e => setDocForm(f => ({ ...f, emissao: e.target.value }))}
                    className="input text-sm"
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <PdfUploadButton
                  ...
                  onUploaded={url => setDocForm(f => ({ ...f, arquivo: url }))}
                  onClear={() => setDocForm(f => ({ ...f, arquivo: '' }))}
                />
              </div>
              <button
                type="submit"
              ...
              >
                {loadingAdd ? 'Salvando...' : 'Salvar Documento'}
              </button>
            </form>
          </div>
          ...
          <div>
            {docs.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full min-h-[140px] text-neutral-400">
                <FileText size={28} className="mb-2 text-neutral-300" />
                <p className="text-xs">Nenhum documento cadastrado</p>
              </div>
            ) : (
              <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
                {docs.map((d: DocumentoSocio) => {
                    ...
                    >
                      <FileText
                        ...
                        className={`flex-shrink-0 mt-0.5 ${
                          expired ? 'text-red-400' : expiring ? 'text-orange-400' : 'text-neutral-400'
                        }`}
                      />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          {(expired || expiring) && (
                            <AlertTriangle size={11} className={expired ? 'text-red-500' : 'text-orange-500'} />
                          )}
                          <p className="text-xs font-medium truncate">{d.nome}</p>
                          {d.arquivo && (
                            <a
                              href={docUrl(d.arquivo)}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-0.5 text-[11px] text-primary-600 hover:underline font-medium shrink-0"
                            >
                              <ExternalLink size={10} /> Ver PDF
                            </a>
                          )}
                        </div>
                        <div className="flex gap-3 mt-0.5 text-[11px] text-neutral-400">
                          {d.emissao && <span>Emissão: {formatDate(d.emissao)}</span>}
                          {d.validade && (
                      ...
                      <button
                        onClick={() => { if (confirm('Remover documento?')) onRemoveDoc(d.id); }}
...
export default function EmpresaPage() {
  ...
  const addDocSocioMut = useMutation({
    mutationFn: ({ socioId, data }: { socioId: string; data: any }) =>
      api.post(`/rh/empresa/socios/${socioId}/documentos`, data),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['empresa'] }); toast.success('Documento adicionado!'); },
    onError: () => toast.error('Erro ao salvar documento do sócio'),
  });

  const removeDocSocioMut = useMutation({
    mutationFn: (id: string) => api.delete(`/rh/empresa/socios/documentos/${id}`),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          <h3 className="font-semibold mb-4 flex items-center gap-2">
            <Building2 size={18} className="text-primary-700" /> Dados Gerais
          </h3>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setShowSocio(true)} className="btn-secondary text-xs">
            ...
            <div className="space-y-2">
              {empresa.socios.map((s) => (
                <SocioCard
                  key={s.id}
                  socio={s}
                  onRemoveSocio={() => { if (confirm(`Remover ${s.nome}?`)) removeSocioMut.mutate(s.id); }}
                  onAddDoc={(socioId, data) => addDocSocioMut.mutate({ socioId, data })}
                  onRemoveDoc={(id) => removeDocSocioMut.mutate(id)}
                  loadingAdd={addDocSocioMut.isPending}
                />
              ))}
            </div>
          )}
            ...
            <div className="bg-primary-50 border border-primary-200 rounded-xl p-4">
              ...
              <form onSubmit={handleSaveDoc} className="space-y-3">
                <div>
                  ...
                  <input value={docForm.nome} onChange={e => setDocForm({ ...docForm, nome: e.target.value })} className="input" placeholder="Ex: Alvará, Licença, Certidão..." />
                </div>
                <div>
                ...
                <div>
                  ...
                  <PdfUploadButton currentUrl={docForm.arquivo} onUploaded={(url) => setDocForm({ ...docForm, arquivo: url })} onClear={() => setDocForm({ ...docForm, arquivo: '' })} />
                </div>
                      ...
                      <div key={d.id} className={`p-3 rounded-lg border flex items-start gap-2 ${expired ? 'bg-red-50 border-red-200' : expiring ? 'bg-orange-50 border-orange-200' : 'bg-neutral-50 border-neutral-200'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {d.arquivo && (
                              <a href={docUrl(d.arquivo)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-0.5 text-[11px] text-primary-600 hover:underline font-medium">
